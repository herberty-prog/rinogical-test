...
async function genOyou(level, sysOverride) {
  const sysDefault =
  ...
  const sys = sysOverride || sysDefault;
...
/* ===========================================================================
   数学の単元（正負の数 / 文字式 / …）を「データ」として1か所にまとめる
   - 新しい単元はここに足すだけ。Battle はこの設定を読んで問題を出し分ける。
   - 将来の小学生版（B案）も、この単位で中身を差し替えれば複製しやすい。
   =========================================================================== */

/* ----- 文字式 v1：式の値（代入）。答えはすべて整数 ----- */
/* 超基本：x に数を代入して値を求める問題を無限生成（1次・係数は正） */
function genKihon_moji(level) {
  const vars = ["x", "a", "y"];
  const vname = vars[rint(0, vars.length - 1)];
  const k = rint(1, Math.min(3 + level, 6)); // 係数（正）
  const b = rint(-9, 9);                       // 定数
  const v = rint(-6, 6);                        // 代入する値
  const coef = (k === 1) ? vname : `${k}${vname}`;
  const cons = (b === 0) ? "" : (b > 0 ? ` + ${b}` : ` - ${-b}`);
  return { q: `${vname} = ${v} のとき　${coef}${cons} の値`, a: k * v + b, kind: "超基本" };
}

/* 標準プール（代入。2変数・2乗・負の係数で歯ごたえ。全問 答えは整数・検算済み） */
const MOJI = [
  { q: "x = 3 のとき　2x + 1 の値", a: 7 },
  { q: "x = -4 のとき　x + 9 の値", a: 5 },
  { q: "a = 5 のとき　3a - 8 の値", a: 7 },
  { q: "x = -2 のとき　5x の値", a: -10 },
  { q: "x = 4 のとき　-2x + 5 の値", a: -3 },
  { q: "y = -3 のとき　-y + 2 の値", a: 5 },
  { q: "a = -6 のとき　-a - 1 の値", a: 5 },
  { q: "x = 7 のとき　2x - 10 の値", a: 4 },
  { q: "x = -3 のとき　x² の値", a: 9 },
  { q: "x = 2 のとき　x² + 1 の値", a: 5 },
  { q: "a = -2 のとき　a² - 3 の値", a: 1 },
  { q: "x = -1 のとき　3x² の値", a: 3 },
  { q: "x = 3, y = -2 のとき　2x + y の値", a: 4 },
  { q: "x = -1, y = 3 のとき　4x - y の値", a: -7 },
  { q: "a = 5, b = -4 のとき　a + b の値", a: 1 },
  { q: "x = -2, y = -3 のとき　x - y の値", a: 1 },
  { q: "x = 6, y = 2 のとき　x - 3y の値", a: 0 },
  { q: "a = -4, b = 5 のとき　2a + b の値", a: -3 },
];

/* 応用：AIに代入の文章題を作らせる（答えは整数） */
const MOJI_OYOU_SYS =
  "あなたは中学1年生（東京書籍・文字式／式の値）の数学問題を作る出題者です。" +
  "x や a に数を代入して値を求める、少し考える文章題を1問だけ作ってください。" +
  "答えが必ず1つの整数になること。出力はJSONのみ。前置き・マークダウン・コードフェンス禁止。" +
  'フォーマット: {"q":"問題文（短く）","a":整数,"choices":[4つの整数。aを必ず含む]}';

/* AI解説（文字式・代入用）。先生は数学共通の「マギ先生」 */
const EXPLAIN_MOJI =
  `あなたは中学1年生のための、やさしい数学の先生「${TEACHER}」です。` +
  "数式を操る魔法使いで、眼鏡と杖を持っています。魔法学校の先生らしい言葉を少しだけ使います（使いすぎない）。" +
  "解説スタイル：①短文で ②箇条書きは行頭に「・」だけ ③むずかしい漢字にはふりがな ④まず良いところを褒める ⑤絶対に責めない・否定しない。" +
  "重要：マークダウン記号（** や * や # や ```）は絶対に使わない。" +
  "文字式（式の値・代入）の単元。文字に数を入れて計算する『代入（だいにゅう）』の考え方を中心に。" +
  "ポイント：2x は 2×x のこと（かけ算の記号がかくれている）／負の数を代入するときは ( ) をつける（例：x=-3 のとき x² =(-3)²）。" +
  "答えそのものより『手順』を伝える。絵文字は1〜2個までOK。毎回みじかく。";

/* 文字式のローカル解説フォールバック（電波が悪いとき用） */
function localExplainMoji(p) {
  return [
    "ナイスチャレンジ！👏 いっしょに見直そう。",
    "・文字に数を入れる『代入（だいにゅう）』だよ。",
    "・2x は 2×x のこと。かけ算の記号がかくれているよ。",
    "・マイナスを入れるときは ( ) をつけてね。例：x=-3 → (-3)。",
    `・答えは ${p.a} になるよ。`,
    "もう一回いける！🌟",
  ].join("\n");
}
...
/* 単元データ本体。Battle はここを見て問題・ボス名・解説を切り替える */
const MATH_UNITS = {
  seifu: {
    name: "正負の数", bossName: "ダーク・ナンバー",
    tokens: SUBJECT_META.math.tokens,
    genKihon: genKihon, pool: HYOJUN,
    genOyou: (lv) => genOyou(lv),
    explain: EXPLAIN_SYSTEM, localExplain: localExplain,
  },
  moji: {
    name: "文字式", bossName: "シャドウ・レター",
    tokens: ["x", "a", "2x", "y", "-3", "x+1", "5", "-x"],
...
const MATH_UNIT_KEYS = ["seifu", "moji", "houtei"];
...
const STR = {
  ja: {
    ...
    sealBroken: "封印を解いた！",
    guardianReturned: "試練の間の守護者がもどってきた。",
    wellDone: (n) => `よくやったね、${n}！`,
    clear: "クリア",
...
export default function Rinogical() {
  ...
  const [unit, setUnit] = useState("seifu");       // 数学のいまの単元（正負の数 / 文字式 …）
  ...
  return (
    <div style={{ ...font, minHeight: 620, color: C.ink, position: "relative", overflow: "hidden",
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", position: "relative" }}>
        ...
        {screen === "trials" && <Home char={character} level={prog.level} coins={player.coins} nickname={nickname} onBoss={(i) => { setUnit(MATH_UNIT_KEYS[i] || "seifu"); setBattleReturn("trials"); setScreen("battle"); }} onBack={() => setScreen("home")} />}
        {screen === "battle" && (
          <Battle char={character} player={player} setPlayer={setPlayer} subjectKey={subject} unitKey={unit} prog={prog} nickname={nickname}
  ...
  return (
    <div style={{ animation: "rino-pop .4s ease", position: "relative", minHeight: 520 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {rooms.map((r, i) => (
          <button key={i} className="rino-btn" disabled={!r.open} onClick={r.open ? () => onBoss(i) : undefined}
...
function Battle({ char, player, setPlayer, subjectKey, unitKey, prog, nickname, onWin, onHome }) {
  // 数学のときは単元（正負の数 / 文字式 …）の設定 U を読む。英語のときは U=null。
  const U = subjectKey === "math" ? (MATH_UNITS[unitKey] || MATH_UNITS.seifu) : null;
  const meta = U
    ? { ...SUBJECT_META.math, bossName: U.bossName, tokens: U.tokens, explain: U.explain }
    : (SUBJECT_META[subjectKey] || SUBJECT_META.math);
  ...
  async function nextProblem() {
    ...
    if (subjectKey === "english") {
      ...
      const roll = Math.random();
      try {
        if (lv >= 5 && roll < 0.5) p = await U.genOyou(lv);
        else if (lv >= 3 && roll < 0.7) p = genFromPool(U.pool);
        else p = U.genKihon(lv);
      } catch { p = genFromPool(U.pool); }
      p.type = "number";
  ...
  async function fetchExplain(extra) {
    ...
    try {
      ...
      setExplain(subjectKey === "english" ? localExplainEn(problem) : (U ? U.localExplain(problem) : localExplain(problem)));
